            ...
            >
                    ...
                    <AnimatePresence custom={direction}>
                        {items.map((item, index) => {
                            ...
                            return (
                                ...
                                >
                                    <img src={item.image} alt={item.title} className="w-full h-full object-cover grayscale opacity-90 mix-blend-multiply pointer-events-none" />
                                </motion.div>
                            );
                        })}
                    </AnimatePresence>
                ...
                <div className="col-start-1 md:col-start-2 row-start-2 flex flex-col justify-center min-h-[160px] text-center md:text-left mt-12 md:mt-0">
                    <AnimatePresence mode="wait">
                        ...
                        >
                            <h3 className="text-2xl font-bold font-sans tracking-tight text-charcoal">{activeItem.title}</h3>
                            <p className="text-xl font-drama italic text-clay mt-1">{activeItem.role}</p>
                            <p className="text-base leading-relaxed text-charcoal/80 mt-4 font-mono">"{activeItem.description}"</p>
                        </motion.div>
                    </AnimatePresence>
                </div>
                ...
                {showNavigation && items.length > 1 && (
                    <div className="col-start-1 md:col-start-2 row-start-3 flex items-center gap-4 mt-6">
                        <button disabled={activeIndex === 0} onClick={handlePrev}
                            className={cn("flex items-center justify-center w-12 h-12 rounded-full border border-charcoal/20 bg-cream transition-all",
                                activeIndex === 0 ? "opacity-30 cursor-not-allowed" : "hover:bg-charcoal/5 hover:scale-105"
                            )}>
                            <ArrowLeft className="w-5 h-5 text-charcoal" />
                        </button>
                        <button disabled={activeIndex === items.length - 1} onClick={handleNext}
                            className={cn("flex items-center justify-center w-12 h-12 rounded-full border border-charcoal/20 bg-cream transition-all",
                                activeIndex === items.length - 1 ? "opacity-30 cursor-not-allowed" : "hover:bg-charcoal/5 hover:scale-105"
                            )}>
